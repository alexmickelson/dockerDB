...
--check if added feature of curent effective bid and top buyer id columns in auction work
	insert into zharbor.test select 53.1,'auctionTopBuyerid', case (select a.topBuyerid from zharbor.auction a where a.id = 1) when 2 then 'PASS' else 'FAIL' END testStatus;
	insert into zharbor.test select 53.2,'auctionTopEffBid', case (select a.topEffBid from zharbor.auction a where a.id = 1) when 5.01 then 'PASS' else 'FAIL' END testStatus;
