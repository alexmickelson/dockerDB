...
delete zharbor.test
	...
	insert into zharbor.test select 0.1, 'mismatchedAcution', case (select count(*) from zharbor.auction a left outer join zharbor.seller s on (a.sellerid = s.id) where s.id=null or a.sellerid != s.id) when 0 then 'PASS' else 'FAIL' end;
